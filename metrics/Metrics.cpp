...
{
    std::unique_lock<std::mutex> lock(metricVectorMutex);
    doRemoveMetric(metricName);
}


//
// caller MUST acquire the metricVectorMutex 1st
void MetricsReporter::doRemoveMetric(const std::string &metricName)
{
...
void MetricsReporter::updateSinkMeasurementsToReport(SinkInfo &sinkInfo)
{
    ...
    if (sinkInfo.metricsChanged)
    {
        ...
        // acquire the metric vector mutex since we are iterating over it possibly making changes
        std::unique_lock<std::mutex> lock(metricVectorMutex);
        ...
        if (!sinkInfo.reportMeasurements.empty())
        {
            for (auto const &measurementInfo : sinkInfo.reportMeasurements)
            {
                ...
                for (auto const &metricIt : metrics)
                {
                    ...
                    if (!pMetric)
                    {
                        doRemoveMetric(metricIt.first);  // we already have the mutex
                        break;
                    }
