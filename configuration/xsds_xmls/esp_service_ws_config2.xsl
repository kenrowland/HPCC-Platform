...
<xsl:import href="esp_service_WsSMC.xsl"/>
    ...
    <xsl:template match="EspBinding">
     ...
     <xsl:variable name="serviceNode" select="/Environment/Software/EspService[@name=$espServiceName and @name=current()/@service and Properties/@type='WsSMC']"/>
        <xsl:apply-templates select="$serviceNode" mode="ws_config2">
    ...
    <!-- WS-config2 -->
    <xsl:template match="EspService" mode="ws_config2">
        <xsl:param name="bindingNode"/>
        <xsl:param name="authNode"/>

        <xsl:variable name="serviceType" select="'ws_config2'"/>
        ...
        <xsl:variable name="bindType" select="'ws_config2SoapBinding'"/>
        <xsl:variable name="servicePlugin">
            <xsl:call-template name="defineServicePlugin">
                <xsl:with-param name="plugin" select="'ws_config2'"/>
            </xsl:call-template>
        </xsl:variable>
        <EspService name="{$serviceName}" type="{$serviceType}" plugin="{$servicePlugin}" folderName="config2" widgetName="Config2Widget" shortName="Config2" longName="Config2 Plugin"/>
        <EspBinding name="{$bindName}" service="{$serviceName}" protocol="{$bindingNode/@protocol}" type="{$bindType}"
            ...
            <xsl:call-template name="bindAuthentication">
                ...
                <xsl:with-param name="service" select="'ws_config2'"/>
            </xsl:call-template>
        </EspBinding>
    </xsl:template>
</xsl:stylesheet>
